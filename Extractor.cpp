        ...
        while((time.empty() || date.empty() || sender.empty() || msg.empty())&&(i!=file.fullText.end())) {
            ...
            for (auto j = currentString.begin(); j != currentString.end(); ++j) {
                if (*j == "class=\"copyable-text\"") {//this is the right string
                    ...
                    tmpString = "";
                    for (int q = 3; j + q != currentString.end()-1; ++j) {
                        tmpString += *(j + q) + " ";
                    }
                    /*k = tmpString.find(":");
                    for (int q = 0; q < k; ++q)
                    ...
                    */
                    tmpString.erase(tmpString.end()-2,tmpString.end());
                    sender = tmpString;
                    break;
                }
                else if((*(j) == "dir=\"ltr\"")){//from this/the next one/two lines after the text begins
                    ...
                    tmpString.erase(tmpString.begin(), tmpString.begin()+tmpString.find("<span>")+6);
                    auto it = std::find(tmpString.begin(),tmpString.end(),'<');
                    if(it != tmpString.end())
